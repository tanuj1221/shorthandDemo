...
const Storage = () => {
  ...
  const fetchStorage = async (keepCurrentFolder = false) => {
    try {
      setLoading(true);
      const response = await axios.get('https://dev.shorthandexam.in/api/storage/structure', {
  ...
  const handleCreateFolder = async () => {
    ...
    try {
      const response = await axios.post('https://dev.shorthandexam.in/api/storage/folder', {
  ...
  const uploadFiles = async (files, isFolder = false) => {
    ...
    try {
      ...
      await axios.post('https://dev.shorthandexam.in/api/storage/upload', formData, {
        withCredentials: true,
  ...
  const handleDeleteFolder = async (folderId) => {
    ...
    try {
      await axios.delete(`https://dev.shorthandexam.in/api/storage/folder/${folderId}`, {
        withCredentials: true
      });
  ...
  const handleDeleteSelectedFiles = async () => {
    ...
    try {
      for (const fileId of selectedFileIds) {
        await axios.delete(`https://dev.shorthandexam.in/api/storage/file/${fileId}`, {
          withCredentials: true
        });
      }
