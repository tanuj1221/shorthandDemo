...
async function generateReportNoPendingWaiting() {
    let connection;
    try {
        ...
        rows.forEach(row => {
            ...
            // FILTER LOGIC: Exclude 'waiting' AND 'pending' AND batchNo 100
            ...
            // Also exclude batchNo 100
            if (row.batchNo === '100' || row.batchNo === 100) {
                excludedCount++;
                return;
            }

            // Group by UTR or fallback
            let key;
        ...
        if (excelRows.length > 0) {
            ...
            xlsx.utils.book_append_sheet(wb, ws, "Revenue_Final");
            const fileName = "Revenue_Report_No_Pending_Waiting.xlsx";
...
generateReportNoPendingWaiting();
